        ...
        <div class="navbar navbar-fixed-top">
            <div class="navbar-inner">
                <div class="container-fluid">
                    <span class="brand" >&nbsp;&nbsp;<font size="5"><b>江 湖 一 隅</b></font></span>
                    <ul class="nav user_menu pull-right">
                        ...
                        <#if (loginUser??)><!--如果user存在-->
                            <img src="user?action=headPic&userid=${loginUser.userid}" alt="头像损坏" class="user_avatar" />
                            ${loginUser.username}
                        <#else>
                            ...
                            <#if (loginUser??)>
                                <li><a href="#post" title="灌水" data-toggle="modal"
                                       ...
                                       onclick="javascript:addz();document.getElementById('submenu').innerHTML='灌水'">发布新帖</a>
                                </li>
                                <li class="divider"></li>
                                <li><a href="user?action=logout"
                                       onclick="javascript:document.getElementById('submenu').innerHTML=''">退出当前用户</a>
                                </li>
                            <#else>
...
<script type="text/javascript">
    $(function() {
		// 获得cookie
        var username = decodeURI($.cookie('myusername'));
        var password = decodeURI($.cookie('mypassword'));
        $('#username').val(username);//设置文本框
        $('#password').val(password);
        //选中【个人理解不应该选中，选中就死循环了】
//        $("input[name='sun']").attr("checked", "checked");
    ...
    //删除从贴，返回主帖的全部从帖
    function del(artid,rootid,duid) {
        $.ajax({
            type: "post",
            url: "article",
            data:"action=delCArt&artid="+artid+"&rootid="+rootid,
            dataType: "text json",
            statusCode:{
                200:function(data){ //返回的全部从帖
                    showhuitie($('#userid').val(),duid,data);
                }
            }
        });
    }
    ...
    */
    function add() {
        var txt=editor.html();//得到帖子内容数据
        if ($('#title').val() =='') {   //判断标题是否为空
        ...
        $('#content').val(txt);//获取textarea的值  ,赋值到hidden
        /*
        ...
        if($("#postrootaction").val()==='reply'){//回帖使用异步，否则不能定位到model
            $.ajax({
                ...
                success : function(data){
                    //先退出发帖窗体，再显示回帖查询model
                    $('#rshow').modal('show');
                    $('#post').modal('hide');
                    // 浏览用户id 主帖用户id 从帖集合list,主帖标题title
                    showhuitie($('#userid').val(),data.userid,data);
                }
            });
        }
    }
    ...
    function addc() {
        ...
        $('#rshow').modal('hide');  //rshow 显示从帖的窗体
        //清空title和content两个控件
        $('#title').val("");//清空标题内容
        $('#content').val("");//清空内容框内容
        $('#post').modal('show'); //post 帖子编辑器窗体
    }

    /*
        uid : 浏览用户的userid
        duid : 发主帖的用户userid
        data : 从异步返回来的json，即该主帖的从帖集合、主帖标题
    */
    function showhuitie(uid,duid,data){  //在model上显示回帖
        // 清空所有控件内容
        $('#span_title').html("");
        $('#tab_head').empty();
        $('#tab_content').empty();
        ...
        if (data.list==''){ //没有从帖
            content =
                    "<div class='tab-pane active' id='tab_l"+0+"'><br/><p>"+"该帖子没有回复"+"</p></div>";
        }
        // 遍历每个从帖
        $(data.list).each(function(i){
            /*
                功能：增加删除从帖按钮
                判断条件：浏览用户是发主帖的用户或浏览用户是发从帖的用户，才能删除
             */
            if(uid==duid||uid==data.list[i].userid){
                console.log("内if："+"浏览用户id:"+uid+",主帖用户id:"+duid+",从帖用户id:"+data.list[i].userid);
                footer="<div id='foot"+i+"' > "+
                        "	<a class='btn btn-danger'  href='#' "+
                        "onclick='javascript:del("+data.list[i].artid+","+data.list[i].rootid+","+duid+")'>删除 </a>"+
                        "</div> ";
            } else{
                console.log("内else："+"浏览用户id:"+uid+",主帖用户id:"+duid+",从帖用户id:"+data.list[i].userid);
                footer="<div></div>";
            }
    ...
    /*
        ...
        duid: 发主帖的用户userid
     */
    function rshow(artid,uid,duid) { //显示回帖的ajax
        //把该主帖的id值付给rshow model下的隐藏字段
        $("#rshowid").val(artid);
        //把该主帖的id值付给post model下的隐藏字段，代表该贴是子贴
        $("#postrootid").val(artid);
        $.ajax({
            type: "post",
            url: "article",
            data:"action=queryCArt&rootid="+artid,
            dataType: "text json",
            success : function(data){   //回调，后端数据传回
                //显示回帖内容，并且显示在model窗体中
                showhuitie(uid,duid,data);
            }
        });
    }
    function upload() {
